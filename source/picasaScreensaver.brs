...
Sub RunScreensaver()
    ...
    if not oa.linked() or sstype=0 then
        rsp=picasa.ExecServerAPI("featured?max-results=300&v=2.0&fields=entry(media:group(media:content))&imgmax=400", invalid)
    else
        rsp=picasa.ExecServerAPI("?kind=photo&max-results=300&v=2.0&fields=entry(media:group(media:content))&imgmax=400")
    end if
    ...
    while(true)
        ...
        HandleCanvasPort(port, 3000, canvas)
...
Sub RunScreenSaverSettings()
    ...
    if linked then
        screen.AddParagraph("Choose the type of photos to show in you screensaver")
